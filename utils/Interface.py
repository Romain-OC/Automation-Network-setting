...
class Interface:
   ...
   def configInterface(self,listMac,address):
      ...
      if self.chemin == "/etc/network/interfaces":
         ...
         mon_fichier = open('/etc/network/interfaces',"a")
         i=0
         while i<(len(listMac)-1):
         ...
         i=0
         while i<(len(listMac)-1):
            shutil.copy('ifcfg-eth'+i, '/etc/sysconfig/network-scripts/')
            mon_fichier = open('/etc/sysconfig/network-scripts/ifcfg-eth'+i,"a")
            chaine = "#DEVICE=eth0"
            for ligne in mon_fichier :
                           if chaine in ligne :
                                 ligne = "DEVICE=eth"+i
